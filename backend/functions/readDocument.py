...
def extract_PO_info(pdf_path):
    ...
    for page_number, page_image in enumerate(pages):
        ...
        #     if word == 'Full Description':
                ...
                x1, y1 = left, top + height  # Left bottom side of the word
                x2, y2 = left + width, top + height + 300  # Expand 1000 units to the right, keeping height the same
                ...
                cleaned_text = text_in_area_description.replace('\n', ' ')
        ...
        for i, word in enumerate(data['text']):
            if word == 'QTY':
                ...
                x1, y1 = left - 10, top + height  # Bottom-left corner of the word
                x2, y2 = left + width + 100, top + height + 300  # Define a larger 
...
if __name__ == "__main__":
    ...
    # pdf_path = os.path.join(script_dir, 'Order 9100062137.pdf') 

    try:
        extract_order_no(pdf_path)
    except Exception as e:
        print(f"Error extracting order number 3: {e}")
        result["order_no_error"] = str(e)

    try:
        extract_PO_info(pdf_path)
    except Exception as e:
        print(f"Error extracting PO info 3: {e}")
        result["po_info_error"] = str(e)
